...
router.get("/", async (req, res) => {
    try {
        ...
        const result = await pool.query(
            "SELECT id, table_number, capacity, status, waiter_id FROM tables WHERE branch_id = $1 ORDER BY table_number ASC",
            [branchId]
        );
...
router.get("/available", async (req, res) => {
    try {
        const branchId = req.user.branch_id;
        const result = await pool.query(
            "SELECT id, table_number, capacity FROM tables WHERE branch_id = $1 AND status = 'available' ORDER BY table_number ASC",
            [branchId]
        );
        res.json({ success: true, data: result.rows });
    } catch (err) {
...
router.post("/", allowManager, async (req, res) => {
    ...
    try {
        const result = await pool.query(
            "INSERT INTO tables (branch_id, table_number, capacity, status) VALUES ($1, $2, $3, $4) RETURNING *",
...
router.put("/:id", allowManager, async (req, res) => {
    ...
    try {
        const result = await pool.query(
            "UPDATE tables SET table_number = COALESCE($1, table_number), capacity = COALESCE($2, capacity), status = COALESCE($3, status), updated_at = NOW() WHERE id = $4 AND branch_id = $5 RETURNING *",
...
router.put("/:id/status", allowManager, async (req, res) => {
    ...
    try {
        const result = await pool.query(
            "UPDATE tables SET status = $1, updated_at = NOW() WHERE id = $2 AND branch_id = $3 RETURNING *",
            [status, id, branchId]
        );
        if (result.rows.length === 0) {
            return res.status(404).json({ success: false, error: "Table not found" });
