...
// ✅ Role hierarchy — platform_admin sits above everyone else
const roleHierarchy = {
  'kitchen': 1,
  'waiter': 2,
  'cashier': 3,
...
// Allowed roles for validation (signup, user creation, etc.)
...
export const protect = async (req, res, next) => {
  ...
  try {
    ...
    // ✅ NEW: reject pending / rejected / inactive users even if they hold a valid JWT
    if (user.status !== 'active' || user.is_active !== true) {
      const code =
        user.status === 'pending'   ? 'ACCOUNT_PENDING_APPROVAL'
      : user.status === 'rejected'  ? 'ACCOUNT_REJECTED'
      : 'ACCOUNT_INACTIVE';
    ...
    // ✅ NEW: enforce company approval for non platform_admin users
    if (user.company_id && user.role !== 'platform_admin') {
      const companyCheck = await query(
        `SELECT is_approved FROM companies WHERE id = $1`,
        [user.company_id]
      );

      if (companyCheck.rows.length > 0 && companyCheck.rows[0].is_approved === false) {
        return res.status(403).json({
          success: false,
          code: 'COMPANY_PENDING',
          error: 'Company awaiting approval.',
        });
      }
    }
...
// ✅ NEW: strict platform-admin guard — no hierarchy
export const allowPlatformAdmin = (req, res, next) => {
